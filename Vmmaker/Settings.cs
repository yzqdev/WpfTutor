...
namespace Vmmaker {
    ...
    public static class Settings {
        private const string ROOT = @"SYSTEM\CurrentControlSet\Control\Session Manager\Environment";
         
        private static RegistryKey GetRoot() {
            RegistryKey? root = Registry.LocalMachine.OpenSubKey(ROOT, true);

            if (root == null)
                root = Registry.LocalMachine.CreateSubKey(ROOT);
        ...
        public static void Set(string name, object value,RegistryValueKind kind) {
            if(value == null) {
                Remove(name);
                return;
            }
            ...
            GetRoot().SetValue(name, value,kind);
        }
        ...
        public static void SetDefault(string name, object value) {
            if (!Has(name))
                Set(name, value,RegistryValueKind.String);
        }
